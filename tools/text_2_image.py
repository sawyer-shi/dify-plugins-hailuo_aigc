...
class Text2ImageTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        """MiniMax text-to-image tool."""
        logger.info("Starting text-to-image task (MiniMax)")
        ...
        try:
            ...
            style_type = tool_parameters.get("style_type")
            style_weight = tool_parameters.get("style_weight")
            ...
            if style_type:
                style_payload = {"style_type": style_type}
                if style_weight not in (None, ""):
                    style_payload["style_weight"] = float(style_weight)
            ...
            yield self.create_text_message(f"🖼️ 返回格式: {response_format}")
